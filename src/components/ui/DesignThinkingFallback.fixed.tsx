...
const DesignThinkingFallback: React.FC = () => {
  ...
  // Track animation state (unused directly, but needed for state updates)
  const [, setAnimationsActive] = useState(true);
  ...
  const getResponsivePositions = () => {
    // Perfect equilateral triangle positions - matematycznie idealne współrzędne
    const defaultPositions = {
      empathy: { x: "50%", y: "25%" },             // Top center - dokładnie na górze środka
      reasoning: { x: "28.87%", y: "75%" },        // Bottom left - dokładna pozycja wyliczona matematycznie (50% - 50%*cos(60°))
      materialization: { x: "71.13%", y: "75%" }   // Bottom right - dokładna pozycja wyliczona matematycznie (50% + 50%*cos(60°))
    };
  ...
  useEffect(() => {
    ...
    if (!isPaused) {
      ...
      setAnimationsActive(true);
        // Start animations with default animation definition
  ...
  const renderTriangleLines = () => {
    return triangleLines.map(({ from, to }) => {      const fromNode = nodes[from];
      const toNode = nodes[to];
      const isActive = fromNode.isSelected && toNode.isSelected;
      
      // Use a single opacity value that considers multiple factors
      const opacity = isPaused ? 0.5 : (isActive ? 1 : 0.7);
        ...
        {nodes.map((node) => (
          <div key={node.id} className="absolute node-container" style={{ 
            ...
            <motion.div
              ...
              animate={{
                scale: node.isSelected ? 1.15 : node.isHovered ? 1.1 : (node.isActive ? [1, 1.08, 1] : 1),
                rotate: node.isHovered && !node.isSelected ? [0, 6, -6, 0] : 0
              }}
              transition={{
                duration: node.isHovered && !node.isSelected ? 0.6 : 2.2,
                repeat: (node.isActive && !node.isSelected) || (node.isHovered && !node.isSelected) ? Infinity : 0,
                repeatType: "reverse",
                ease: "easeInOut"
              }}
            >
