...
const ContactTypeCard = styled.button`
  padding: 30px;
  text-align: left;
  background: ${props => props.$active ? 'var(--text)' : 'var(--background)'};
  color: ${props => props.$active ? 'var(--background)' : 'var(--text)'};
  border: 2px solid ${props => props.$active ? 'var(--text)' : 'var(--border)'};
  cursor: pointer;
  transition: all 0.3s ease;
  
  &:hover {
    border-color: var(--text);
    background: ${props => props.$active ? 'var(--text-secondary)' : 'var(--surface)'};
  }
`;
...
const Input = styled.input`
  ...
  border: none;
  border-bottom: 2px solid ${props => props.$hasError ? '#dc3545' : 'var(--border)'};
  color: var(--text);
...
const SubmitButton = styled.button`
  ...
  text-transform: uppercase;
  letter-spacing: 0.1em;
  background: ${props => props.$isLoading ? 'var(--text-muted)' : 'var(--text)'};
